# app_cloud.py - Cloud-Ready Version
...
st.write("ارفع ملف CSV, Excel, TXT أو PDF لتحليل البيانات باستخدام Spark وML على Cluster حقيقي")
...
if uploaded_file is not None:
    ...
    try:
        if ext == "csv":
            ...
            except: df = pd.read_csv(uploaded_file, encoding="latin1")
        elif ext in ["xls","xlsx"]:
            df = pd.read_excel(uploaded_file)
        elif ext == "txt":
            df = pd.read_csv(uploaded_file, delimiter="\t", encoding="utf-8", engine="python")
        elif ext == "pdf":
            text_data = ""
            with pdfplumber.open(uploaded_file) as pdf:
                for page in pdf.pages: text_data += page.extract_text() + "\n"
    ...
    if df is not None:
        ...
        for c in spark_df.columns:
            ...
            missing_list.append([c, (missing_count/total_rows)*100])
        st.subheader("نسبة القيم المفقودة لكل عمود:")
        st.dataframe(pd.DataFrame(missing_list, columns=["Column","Missing (%)"]))
        ...
        if len(date_cols)>0 and len(numeric_cols_list)>0:
            ...
            for nodes in nodes_list:
                ...
                if nodes==1: base_times["TimeSeries"]=elapsed

         for r in ml_results:
            task_name=r["task"].split("_")[0]
            r["speedup"]=base_times.get(task_name,1)/r["time"] if r["time"]>0 else None
        ...
        st.download_button(label="تنزيل نتائج ML كملف CSV",
